 ...
 */
  fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    ...
    val rowsUpdated: List<Int> = rowOrColumn
        .map {  this[it] }
 ...
 */
  ...
  fun GameBoard<Int?>.moveValues(direction: Direction): Boolean {
    ...
    when (direction) {
        Direction.LEFT -> {
            repeat(width+1){
                val rows = this.getRow(it, 1..width)
                moved = moved or moveValuesInRowOrColumn(rows)
            }
        }
        Direction.RIGHT -> {
            repeat(width+1){
                val rows = this.getRow(it, width downTo 1)
                moved = moved or moveValuesInRowOrColumn(rows)
            }
        }
        Direction.DOWN -> {
            repeat(width+1){
                val column = this.getColumn(width downTo 1, it)
                moved = moved or moveValuesInRowOrColumn(column)
            }
        }
        Direction.UP -> {
           repeat(width+1){
                val column = this.getColumn(1..width, it)
                moved = moved or moveValuesInRowOrColumn(column)
            }
        }
    }
