...
class GameBoardImpl<T>(width: Int) : GameBoard<T>, SquareBoardImpl(width) {
    var ce: Array<Array<Pair<Cell, T?>>>
...
//
    ...
    override fun get(cell: Cell): T? {
        return ce[cell.i][cell.j].second
    }

    override fun set(cell: Cell, value: T?) {

        ce[cell.i][cell.j] = ce[cell.i][cell.j].copy(second = value)

    }

    override fun filter(predicate: (T?) -> Boolean): Collection<Cell> {
        var collection = listOf<Cell>()
        for ((indexI, i) in ce.withIndex())
            for ((indexJ, j) in ce.withIndex())
                if (predicate(ce[indexI][indexJ].second)) collection =
                    collection + ce[indexI][indexJ].first
        return collection
    }

    override fun find(predicate: (T?) -> Boolean): Cell? {
        for ((i, e) in ce.withIndex())
            for ((j, e1) in ce.withIndex())
                if (predicate(ce[i][j].second)) return ce[i][j].first

        return null
    }

    override fun any(predicate: (T?) -> Boolean): Boolean {
        return ce.any { it.any { r -> predicate(r.second) } }
    }

    override fun all(predicate: (T?) -> Boolean): Boolean {
        return ce.copyOfRange(1,width+1).all { it.copyOfRange(1,width +1).all { r -> predicate(r.second) } }
...
open class SquareBoardImpl
    (final override val width: Int) : SquareBoard {
    val cells: Array<Array<Cell>>

    init {
        cells = Array((width + 1)) { row -> Array((width + 1)) { col -> Cell(row, col) } }
